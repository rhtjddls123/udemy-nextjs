...
function initDb() {
  ...
  const result = stmt.get({}) as { count: number }; // 빈 객체 전달
  if (result.count === 0) {
    db.exec(`
...
export async function getPosts(maxNumber?: number): Promise<PostType[]> {
  ...
  return maxNumber ? (stmt.all(maxNumber) as PostType[]) : (stmt.all() as PostType[]);
}

export async function storePost(post: NewPostType): Promise<RunResult> {
  const stmt = db.prepare(`
